...
export function CartSheet() {
  ...
  const handleCheckout = async () => {
    ...
    if (totalPrice > walletBalance) {
      toast.error(`رصيد المحفظة غير كافٍ. تحتاج إلى $${(totalPrice - walletBalance).toFixed(2)} إضافية.`);
      return;
    }
    ...
    try {
      ...
      toast.error("فشل في إتمام الشراء. يرجى المحاولة مرة أخرى.");
    } finally {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      ...
      {/* Set side="right" because direction is rtl */}
      <SheetContent side="right" className="w-[300px] sm:max-w-md bg-slate-900 border-l border-teal-900/50 flex flex-col pt-12 z-[100]">
                  ...
                  <div key={item.id} className="flex gap-4 bg-slate-800/50 rounded-lg p-3 border border-slate-700/50">
                    ...
                    <div className="flex-1 flex flex-col justify-between overflow-hidden">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <span className="text-sm font-bold text-teal-400">${item.price.toFixed(2)}</span>
                        <Button
                          variant="ghost"
                          size="icon"
